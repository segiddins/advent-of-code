...
class Particle
	attr_reader :n, :p, :v, :a
	def initialize(n, p, v, a)
		@n, @p, @v, @a = n, p, v, a
	end

	def -(other)
		Particle.new([n, other.n], p - other.p, v-other.v, a-other.a)
	end
	...
	def self.from_s(n, s)
		return unless s =~ /\Ap=<([ 0-9-]+),([ 0-9-]+),([ 0-9-]+)>, v=<([ 0-9-]+),([ 0-9-]+),([ 0-9-]+)>, a=<([ 0-9-]+),([ 0-9-]+),([ 0-9-]+)>\z/

		new(n,
			Vector[$1,$2,$3].map(&:to_i),
			Vector[$4,$5,$6].map(&:to_i),
			Vector[$7,$8,$9].map(&:to_i),
		)
	end
end

particles = DATA.readlines.each_with_index.map {|l,n| Particle.from_s(n, l.chomp) }

#p particles.min_by { |p| p.distance_at(99_999_999_999) }.n

p particles.min_by {|p| [p.distance(p.a), p.distance(p.v), p.distance(p.p)] }.n

0.upto(particles.map(&:a).map {|a| a.to_a.sum(&:abs) * 10 }.max + 100) do |t|
	particles.group_by { |p| p.pos_at(t) }.each do |_, ps|
		particles -= ps if ps.size > 1
	end
end
