...
needed_fields = %w[byr iyr eyr hgt hcl ecl pid]
passports = DATA.read.split(/\n[ \t]*\n/).map { |s| s.scan(/([a-z]{3}):([^\s]+)/).to_h }

validate = {
  'byr' => ->(x) { return unless x =~ /^\d{4}$/; x=x.to_i; x >= 1920 && x <= 2002 },
  'iyr' => ->(x) { return unless x =~ /^\d{4}$/;x=x.to_i; x >= 2010 && x <= 2020 },
  'eyr' => ->(x) { return unless x =~ /^\d{4}$/;x=x.to_i; x >= 2020 && x <= 2030 },
  'hgt' => ->(x) { if x =~ /^(\d+)in$/ then x=$1.to_i; x >= 59 && x <= 76 elsif x =~ /^(\d+)cm$/ then x=$1.to_i; x >= 150 && x <= 193 end },
  'hcl' => ->(x) { /\A#[0-9a-f]{6}\z/.match?(x) },
  'ecl' => ->(x) { %w[amb blu brn gry grn hzl oth].include?(x) },
  'pid' => ->(x) { /\A\d{9}\z/.match?(x) },
}

pp passports.count { |a| needed_fields.all? {|f| a.key?(f) }}
pp passports.count { |a| needed_fields.all? {|f| a.key?(f) && validate[f][a[f]] }}
