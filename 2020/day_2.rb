...
def validate1?(pw)
  _, min, max, l, pw = pw.match(/(\d+)-(\d+) (.): (.+)/).to_a
  c = pw.chars.select { |c| c == l }.size
  c >= min.to_i && c <= max.to_i
end

def validate2?(pw)
  _, p1, p2, l, pw = pw.match(/(\d+)-(\d+) (.): (.+)/).to_a

  (pw[p1.to_i.pred] == l) ^ (pw[p2.to_i.pred] == l)
end

lines = DATA.read.lines

pp lines.select { |l| validate1?(l.strip) }.size
pp lines.select { |l| validate2?(l.strip) }.size
