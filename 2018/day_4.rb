...
data = DATA.each_line.map do |l|
  l =~ /\A\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] (.+)\Z/
  [DateTime.new($1.to_i, $2.to_i, $3.to_i, $4.to_i, $5.to_i), $6]
end.sort_by(&:first)

sleeps = Hash.new {|h,k| h[k] = 0 }
sleeps_by_minute_and_guard = Hash.new {|h,k| h[k] = 0 }
guard = nil
sleep_start = nil
data.each do |time, message|
  case message
  when /\AGuard #(\d+) begins shift/
    guard = $1
  when 'falls asleep'
    sleep_start = time
  when 'wakes up'
    (0..60).each do |min|
      next unless (sleep_start.minute...time.minute).include?(min)
      sleeps_by_minute_and_guard[[min, guard]] += 1
    end
    sleeps[guard] += (time - sleep_start) * 24 * 60
  end
end

guard = sleeps.max_by(&:last).first
minute = sleeps_by_minute_and_guard.select { |(_, g), _| g == guard}.max_by(&:last).first.first

puts guard.to_i * minute

minute, guard = sleeps_by_minute_and_guard.max_by(&:last).first
puts minute * guard.to_i
