...
def parse(str)
  str.readlines.map do |l|
    l =~ /\Aposition=<\s*(-?\d+),\s*(-?\d+)> velocity=<\s*(-?\d+),\s*(-?\d+)>\Z/
    Point.new(Pt.new($1.to_i, $2.to_i), Pt.new($3.to_i, $4.to_i))
  end.to_set
end

def print_points(points)
  x_bounds, y_bounds = bounding_box(points)
  puts '-' * x_bounds.size
  positions = points.map(&:position).to_set
  y_bounds.each do |y|
    x_bounds.each do |x|
      if positions.include?(Pt.new(x, y))
        print '#'
      else
        print '.'
      end
...
def advance(points)
  x_bounds, y_bounds = bounding_box(points)
  points.map do |point|
    point.advanced.tap do |advanced|
      return unless x_bounds.cover?(advanced.position.x) && y_bounds.cover?(advanced.position.y)
    end
  end
end

def bounding_box(points)
  x_bounds = Range.new(*points.map { |pt| pt.position.x }.minmax)
  y_bounds = Range.new(*points.map { |pt| pt.position.y }.minmax)
  [x_bounds, y_bounds]
end
...
loop do
  break unless new_points = advance(points)
  seconds += 1
  points = new_points
end
