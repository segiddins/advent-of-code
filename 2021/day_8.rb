...
def part1
  $lines.flat_map { |l| l.split(' | ', 2).last.split(' ') }.count { |w| [2, 3, 4, 7].include?(w.size) }
end
...
def part2
  $lines.map { |l|
    ...
    (l + r).each { |w| case w.size
    when 2 then mapping[1] << w
    when 3 then mapping[7] << w
    when 4 then mapping[4] << w
    when 7 then mapping[8] << w
    when 5 then [2, 3, 5].each { |n| mapping[n] << w }
    when 6 then [0, 6, 9].each { |n| mapping[n] << w }
    end
    }
    ...
    mapping[5].reject! { |w| (mapping[6].to_a.only.chars - w.chars).size != 1 }
    ...
    rm = mapping.each_with_object({}) do |(k, v), a|
      a[v.to_a.only] = k
    end
