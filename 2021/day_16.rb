...
def read_packet(i)
  ...
  when 4
    vs = ''
    i.gsub!(/^(?:1(\d{4}))*(?:0(\d{4}))/) do |m|
      vs = m
      ''
    end
    v = vs.scan(/.(\d{4})/).map { |g| g }.join.to_i(2)
    ...
    subpackets =
      if ltid == '0'
        ...
        exps = i.size - tlib
        sp = []
        sp << read_packet(i) while i.size > exps
        sp
      else
        Array.new(i.slice!(0, 11).to_i(2)) { read_packet(i) }
      end
