...
class Range
  def until(other)
    Range.new(self.begin, other.begin.pred)
  end

  def from(other)
    Range.new(other.end.succ, self.end)
  end
...
Region =
  Struct.new(:is_on, :x, :y, :z) do
    def run_inst(inst)
      ...
      unless x.overlaps?(inst.x) && y.overlaps?(inst.y) && z.overlaps?(inst.z)
        return
      end
